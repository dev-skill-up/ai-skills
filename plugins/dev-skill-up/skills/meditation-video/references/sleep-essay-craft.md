...
- **Image:** a slightly dim, low-stimulation backdrop (night sky, moonlit water, candlelight). The dimness is **symbolic, not functional** — it signals night and sleep, setting the mood for the listener; that calls for only *slightly* darker than a daytime image, never murky. Keep a recognizable subject with clear contrast (a moonlit horizon, a candle flame, stars) — the backdrop also doubles as the video's thumbnail. Dim scene, discernible subject. **Fetch it from Unsplash** with `assets/fetch_image.sh` — never generate or synthesize a backdrop. **Randomize this choice too:** collect several candidate images that fit the brief and pick one with `shuf`, the same anti-favoritism rule as the topic draw. Credit the photo in the description (`references/publishing-metadata.md`). **That one photograph is the entire visual layer** — no diagrams, no caption panels, no plates, no slideshow. Those are casual-essay machinery; a sleep essay is listened to with eyes closed, so anything worth showing on screen is a sign the prose should carry it instead.
