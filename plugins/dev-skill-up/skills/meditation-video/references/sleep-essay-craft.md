...
- **Image:** a dark, dim, low-stimulation backdrop (night sky, dark water, candlelight) so a phone screen left on doesn't light up the room — **but not *too* dark: it must stay legible as a thumbnail.** The backdrop doubles as the video's thumbnail, and a near-black frame reads as a broken upload in a search-results grid. There should be a recognizable subject with some contrast (a moonlit horizon, a candle flame, stars) that survives being shrunk to thumbnail size; check the candidate at small scale before committing. Dim scene, discernible subject. **Fetch it from Unsplash** with `assets/fetch_image.sh` — never generate or synthesize a backdrop. **Randomize this choice too:** collect several candidate images that fit the brief and pick one with `shuf`, the same anti-favoritism rule as the topic draw. Credit the photo in the description (`references/publishing-metadata.md`). **That one photograph is the entire visual layer** — no diagrams, no caption panels, no plates, no slideshow. Those are casual-essay machinery; a sleep essay is listened to with eyes closed, so anything worth showing on screen is a sign the prose should carry it instead.
