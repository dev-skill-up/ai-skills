...
Run the AI-tell convergence loop (`references/ai-tells.md`) on the finished Markdown draft: every pass — the destructive excision and cut passes, the metrics script, the adversarial and cold reviews, and the independent fact-check — re-run from the top after any fix, until one complete cycle returns zero findings from every pass. That clean cycle is a hard gate, and the gate is enforced in code: write the clean cycle's structured verdicts to `passes.json`, because `generate_segments.py` refuses to render an essay-derived script without that report and checks every pass in it for explicit zero findings. Do not edit the scripts to weaken the check, and do not record a verdict for a pass that did not run — a blocked gate means the loop is not done. Do the loop before `essay_to_segments.py`: fixing prose after generation wastes all the TTS compute.
